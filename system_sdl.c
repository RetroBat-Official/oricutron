...
SDL_Surface* SDL_COMPAT_SetVideoMode(int width, int height, int bitsperpixel, Uint32 flags)
{
...
#ifndef __ANDROID__
	g_window = SDL_CreateWindow("oricutron", g_lastx, g_lasty, g_width, g_height, flags);
#else
	g_window = SDL_CreateWindow("oricutron", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 0, 0, SDL_WINDOW_FULLSCREEN_DESKTOP);
#endif
	if (g_icon)
	...
	{
		g_screen = SDL_CreateRGBSurface(0, g_width, g_height, g_bpp, RMASK, GMASK, BMASK, AMASK);
		...
		g_texture = SDL_CreateTexture(g_renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, width, height);
	}
